...
/**
 * Armazena em um dois ARRAYS os itens que serão INCLU�?DOS ou DELETADOS da Lista Dupla.
 ...
 * Caso contrário o VALUE do item será adicionado ao ARRAY que armazena os valores que serão INCLU�?DOS.
 **/
...
HTMLDualList.prototype.designComponent = function(doc) {
  ...
  // Criar label de descrição principal.
  ...
  // Criar label de descrição dos itens selecionados.
  ...
  // Verificar se o componente tem botão de adicionar opção habilitado.
  if (this.btAddItem) {
    // Criar botão de adicionar opção.
    ...
    // Associar evento ao botão de adicionar opção.
    this.attachEvent(this.addOptionButton, 'click', this.moveOption, this, ["LEFT", "item"]);
  }

  // Verificar se o componente tem botão de adicionar todas as opções habilitado.
  if (this.btAddAll) {
    // Criar botão de adicionar todas as opções.
    ...
    // Associar evento ao botão de adicionar todas as opções.
    this.attachEvent(this.addAllOptionsButton, 'click', this.moveAllOptions, this, ["LEFT", "all"]);
  }

  // Verificar se o componente tem botão de remover todas as opções habilitado.
  if (this.btRemAll) {
    // Criar botão de remover todas as opções.
    ...
    // Associar evento ao botão de remover todas as opções.
    this.attachEvent(this.removeAllOptionsButton, 'click', this.moveAllOptions, this, ["RIGHT", "all"]);
  }

  // Verificar se o componente tem botão de remover opção habilitado.
  if (this.btRemItem) {
    // Criar botão de remover opção.
    ...
    // Associar evento ao botão de remover opção.
...
/**
 * Obtém dois arrays com os labels e valores do select em um lado da lista dupla.
 ...
 * @param query Descrição do item pra procurar.
 **/
HTMLDualList.prototype.searchOption = function(side, query) {
  ...
  if (side == "LEFT") {
    ...
    if (query && query.trim().length > 0) {
      ...
      // Esconder a lista padrão e mostrar a lista de resultados.
      this.leftSelect.className = "d-none"; // Bootstrap
      this.leftSearchSelect.className = this.leftSearchSelectClass;

      // Exibir opções que contenham o texto digitado na lista de resultados.
      this.setOptions(this.leftSearchSelect, arrays.labels, arrays.values, query);
    } else {
      // Esconder a lista de resultados e mostrar a lista padrão.
 ...
 * @param side Lado da lista dupla que o item será associado.
...
/**
 * Limpa os objetos (HTMLInput) que armazenam os valores que serão INCLU�?DOS ou EXCLU�?DOS a cada navegação da página.
 **/
