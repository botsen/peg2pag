...
HTMLElementBase.prototype.setHint = function(hint, element) {
  this.hint = hint;

  let target = element ? element : this.input;

  if (target) {
    let init = target.hint ? false : true;
    if (target.getAttribute("data-original-title")) {
      target.setAttribute("data-original-title", hint.replace(/(?:\r\n|\r|\n)/g, '<br />'));
    } else {
      target.setAttribute("data-toggle", "tooltip"); // Bootstrap
      target.setAttribute("data-html", "true"); // Bootstrap
      target.hint = hint;
      target.title = hint.replace(/(?:\r\n|\r|\n)/g, '<br />');
      if (init) bootstrapInitTooltip(target);
    }
  }
};
...
HTMLElementBase.prototype.design = function(doc, tabControl) {
  ...
  if(verifyObjectType(this, 'HTMLContainer') && this.container){
    if(doc){
  ...
  if(initContainer && !verifyObjectType(this, 'HTMLContainer')){
...
HTMLElementBase.prototype.init = function(tabControl) {
  ...
  if (this.mobileInput) {
    this.attachEvent(this.mobileInput, 'focus', this.focusAction);
    this.attachEvent(this.mobileInput, 'blur', this.blurAction);
    this.attachEvent(this.mobileInput, 'keydown', this.keydownAction);
    this.attachEvent(this.mobileInput, 'click', this.clickAction);
    ...
    this.attachEvent(this.mobileInput, 'keyup', this.keyupAction);
  } else if (this.input) {
    ...
    if (!(verifyObjectType(this,'HTMLLookup')) && !(verifyObjectType(this, 'HTMLCheckBox'))) {
  ...
  if (verifyObjectType(this, 'HTMLGrid')) {
  ...
  this.designMask();
  if(!(verifyObjectType(this, 'HTMLContainer') && this.container && this.doc == null)){
    this.afterInit();
  }
