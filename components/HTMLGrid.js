...
HTMLGrid.prototype.designComponent = function() {
  ...
  if (this.hasPagination) this.createFooterRegisters();
  ...
  dataSource.setTypeOperators("boolean", _boolean);
  ...
  this.iscCanvas = isc.ListGrid.create({
    ...
    dataChanged: function () {
      ...
      if(componentGrid.hasPagination && componentGrid.enableNumberOfRecords && componentGrid.totalLabels)
        componentGrid.totalLabels.setContents(componentGrid.gridend + " de " + componentGrid.totalRows + " " + getLocaleMessage("INFO.GRID_COUNT_RECORDS"));
    },
    wrapCells: false,
    ...
    // Esse método é chamado para executar o evendo de duplo clique, independente de possuí registros.
    doubleClick: function() {
      if (this.getTotalRows() === 0)
        $c(this.ID).executeRowDoubleClick();
    },
...
HTMLGrid.prototype.getRowIgnoreState = function () {
  ...
  if (this.isGrouped()) {
    ...
    row = this.inserting
            ? this.editRow
            : this.getRowDBCursor(this.getRecord(this.editRow)) === -1
              ? this.data.length - 1
...
HTMLGrid.prototype.setColumns = function (cols, width, components) {
  // Responsável por ordenar as colunas da grade mantendo a equivalência com o servidor.
...
/**
 * Método responsável por criar o rodapé com as informações de quantidade de registros do componente.
 * @author Janpier
 */
HTMLGrid.prototype.createFooterRegisters = function () {
  if(this.enableNumberOfRecords){
    this.registers = isc.ToolStrip.create({
        ID: this.id + "_registers",
        width: "100%",
        height: 24,
        members: [
          this.totalLabels = isc.Label.create({
            ID: this.id + "_totalsLabel",
            padding: 3,
            width: "100%",
          })
        ],
      });
    this.components_to_Grid.push(this.registers);
  }
}
...
HTMLGrid.prototype.setShowColumn = function (c, show) {
  ...
  if (comp) {
    ...
    if (this.showGroupSummary && this.showGroupSummaryInHeader && this.isGrouped()) idx = idx - 1;
    if (projectMode.toLowerCase() === 'n')
  ...
  if (this.iscCanvas.getEditRow() !== row) {
    ...
    if (expression.length > 0) {
      ...
      for (var j = 0; j < expressions.length; j++) {
        ...
        if (paint) {
          if (showRowNumbers && (column === 0)) return;
...
HTMLGrid.prototype.completeDeleteRow = function () {
  ...
  if(this.hasPagination) this.iscCanvas.dataChanged();
};
...
HTMLGrid.prototype.refresh = function (updateColumns, keepFocus) {
  ...
  gridSendCommand(url, "_"+this.id+"_refresh", this.div, true, function(resp, grid, updateColumns){
    ...
    if(grid.hasPagination) grid.iscCanvas.dataChanged();
...
HTMLGrid.prototype.filter = function (f, editor) {
  ...
  gridSendCommand(url, "_"+this.id+"_filter", this.div, true, function (resp, grid) {
    eval(resp);
    grid.iscCanvas.dataChanged();
  }, this, null, this.id);
  this.refreshURL = url;
};
...
HTMLGrid.prototype.parseValue = function (cod, value, type, col) {
  ...
  if(type === 'boolean'){
    return value ? this["componentCheck"+cod].valueCheck : this["componentCheck"+cod].valueUnCheck;
  }
  return value;
};
...
HTMLGrid.prototype.parseOperator = function (operator, type, def, cod) {
  ...
  if(def && !equals){
    if(type === "integer" || type === "double")
      ...
      if(this["componentCheck"+cod].valueCheck === "true" || this["componentCheck"+cod].valueCheck === "false")
        return 28;
      else if(!isNaN(this["componentCheck"+cod].valueCheck) && isFinite(this["componentCheck"+cod].valueCheck))
        return 17
    }
    return 2;
  }
...
HTMLGrid.prototype.enableNavigation = function (f, p, n, l) {
  ...
  if (f || p || n || l) this.hasPagination = true;
...
HTMLGrid.prototype.clickGrid = function () {
  ...
  if (this.onclick && this.enabled) {
    var relativeRow = this.isGrouped() ? this.getRowDBCursor() : this.getSelectedRow();
...
function createEditComponents(grid, arr, rowReference, include, editComs) {
  ...
  for (colNum; colNum < fields.length; colNum++) {
    ...
    if (elem) {
      ...
      if (verifyObjectType(elem, 'HTMLLookup') || elem.typeName === 'datetime' || elem.typeName === 'date' || verifyObjectType(elem, 'HTMLImage')) {
        elem.div.style.position = "relative";
      } else elem.div.style.position = "inherit";
      if (verifyObjectType(elem, 'HTMLCheckbox')) {
...
HTMLGrid.prototype.gridKeyPress = function (evt) {
  ...
  if (((!altKey && !ctrlKey && keyCode == 27))) { //Cancelar
    ...
    if ((!this.editable || this.lockEditable) && this.callForm) {
...
HTMLGrid.prototype.checkDoubleClick = function () {
  if (!this.editing && !this.inserting) {
    controller.activeElement = this;
    if (!this.ondblclick || !this.doubleClickOnly) {
      if (this.editable && (!this.lockEditable)) {
        if (this.data.length !== 0)
          this.edit();
 ...
 * O resize só ocorrerá caso o usuário não realizar a costumização das colunas e se não for um acesso pelo dispositivo móvel
 * @author Janpier
 * @param resizeGrid percentual para resize.
 */
HTMLGrid.prototype.resizeColumns = function(resizeGrid) {
  if (!this.userCustomColumnWidth && !isMobile()) {
...
/**
 * Método responsável por atualizar o indíce de paginação do componente.
 * @author Janpier
 */
HTMLGrid.prototype.updatePage = function () {
  this.checkFooterRegisters();
  if(this.hasPagination && this.enableNumberOfRecords && this.totalLabels) this.iscCanvas.dataChanged();
  var elem = document.getElementById(this.id + "_page");
  if(elem) elem.innerHTML = this.gridPage;
};

/**
 * Método responsável por verificar se o rodapé deve ser visível no componente
 * @author Janpier
 */
HTMLGrid.prototype.checkFooterRegisters = function () {
  if(this.hasPagination && this.enableNumberOfRecords && !this.totalLabels) {
    this.createFooterRegisters();
    this.iscCanvas.updateGridComponents();
  } else if (this.enableNumberOfRecords && this.totalLabels && !this.hasPagination) {
    // Remove sempre o último, pois neste caso, a referência do rodapé deve ser sempre a última a renderizada pelo componente.
    this.components_to_Grid.pop();

    this.totalLabels = undefined;
...
/**
 * Método que define o total de linhas de uma grade.
 * Esse método sempre é chamado pelo servidor de aplicações.
 * @author Janpier
 * @param {*} total
 */
HTMLGrid.prototype.setTotalRows = function (total) {
  this.totalRows = total;
  this.updatePage();
};

/**
 * Atualiza as informações de última página.
 * @author Janpier
 * @param {*} total
 */
HTMLGrid.prototype.checkGridEnd = function (total) {
  if (this.isLastPage && total !== this.gridend) this.gridend = total;
};
...
/**
 * Método responsável por limpar a seleção da grade após uma atualização.
 *
 * @author bruno.brasil
 * @param {*} rowNum referência da linha selecionada.
 */
 HTMLGrid.prototype.clearDataSelected = function (rowNum) {
  this.selectRow(0);
  this.clearSelectedRows(rowNum);
  this.currentRow = 0;
};
      ...
      if (xhr.status === 200) {
        ...
        if (this.response !== ''){
          if(callback && typeof callback === "function")
            return callback.call(this, this.responseText, grid, updateColumns);
          eval(this.response);
        }
      }
    }
...
function getGridVerifyColumns(grid, colsPaint) {
  ...
  let isGrouped = grid.isGrouped();
  let isShowGroupSummary = grid.showGroupSummary;
  let isShowGroupSummaryInHeader = grid.showGroupSummaryInHeader;
  grid = grid.iscCanvas;
  let showColumns = grid.fields;
  let allColumns = grid.getAllFields();
  let nCols = [];
  for (let i=0; i < size; i++) {
    let field = allColumns[isGrouped && isShowGroupSummary && isShowGroupSummaryInHeader ? colsPaint[i]+1 : colsPaint[i]];
