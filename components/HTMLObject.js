...
/**
 * Fecha todos os formulários e janelas.
 */
function webrunCloseAllForms() {
  try {
    // Fechar todos os formulários flutuantes.
    closeFormHierarchy(mainform.idForm);

    // Verificar se não é formulário principal.
...
/**
 * Método responsável por verificar se objeto é de uma determinada classe.
 * @param {*} object 
 * @param {*} clazz 
 * @returns Verdadeiro caso a classe seja do objeto, falso caso contrário.
 */
        ...
        switch (event.data.action.toLowerCase()) {
          ...
          case 'wle' :
            if (mainSystemFrame && mainSystemFrame.SESSION_ID && event.data.sessionId &&
              ...
              // Fechar todos os formulários e janelas.
              webrunCloseAllForms();

              // Abrir modal de sessão atualizada.
